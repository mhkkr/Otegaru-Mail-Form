...
class Send
{
    ...
    public function __construct()
    {
        ...
        $result = $this->sendSetup($body, $timestamp);
        $successful = $this->isSuccessful($result);

        $this->sendAfter($body, $result, $timestamp, $successful);
    }
     ...
     * @return string $body
     */
    protected function generateTemplateBody()
    {
        $body = '';
     ...
     * @return string $str
     */
    protected function optimizeKeyword(string $str, array $timestamp, bool $is_tag_remove)
    {
        // 文字列でないならそのまま返す
        if (!is_string($str)) {
            return $str;
    ...
    protected function issueTimeStamp()
    {
        ...
        if (is_writable($file)) {
            $number = file($file);
            if (is_array($number) && isset($number[0]) && preg_match('/\A[0-9]+\z/', $number[0])) {
                $timestamp = intval($number[0]) + 1; // 数値化してプラス1
            }
     ...
     */
    protected function sendSetup(string $body, array $timestamp)
    {
        // 送信結果を記録
        $result = [];
        ...
        foreach (Store::getConfig()['send'] as $key => $val) {
            if ($val['to_email'] && $val['from_email']) {
                ...
                // 送信処理
                if ($this->sendRequest($body, $timestamp, $Qdmail, $val)) {
     ...
     * @return bool
     */
    protected function sendRequest(string $body, array $timestamp, Object $Qdmail, array $config)
    {
        // 宛設定
        $subject       = $this->optimizeKeyword($config['subject'], $timestamp, true);
        $to_email      = $this->optimizeKeyword($config['to_email'], $timestamp, true);
        $to_name       = $this->optimizeKeyword($config['to_name'], $timestamp, true);
        $from_email    = $this->optimizeKeyword($config['from_email'], $timestamp, true);
        $from_name     = $this->optimizeKeyword($config['from_name'], $timestamp, true);
        $replyto_email = isset($config['replyto_email']) ? $this->optimizeKeyword($config['replyto_email'], $timestamp, true) : '';
        $replyto_name  = isset($config['replyto_name']) ? $this->optimizeKeyword($config['replyto_name'], $timestamp, true) : '';
        $header        = $this->optimizeKeyword($config['header'], $timestamp, false);
        $footer        = $this->optimizeKeyword($config['footer'], $timestamp, false);
     ...
     */
    protected function sendAfter(string $body, array $result, array $timestamp, bool $successful)
    {
        // 状態による完了表示へ移動する
        header('Location: ' . Store::getConfig()['setting'][$successful ? 'success_url' : 'failure_url']);
        exit();
    }
     ...
     * @return bool
     */
    protected function isSuccessful(array $result)
    {
        if (empty($result)) {
            return false;
        } else {
